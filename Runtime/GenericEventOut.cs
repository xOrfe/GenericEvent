...
namespace XO.GenericEvent.Runtime
{
    ...
    [Serializable]
    public class GenericEventOut<TOut> : GenericEventBase
    {
        [SerializeField] private string name;
        [SerializeField] private List<string> assignedMethods = new List<string>();
        
        public delegate TOut EventDelegate();
        ...
        public bool Invoke(ref TOut ieArgsOutput)
        {
            if (MyEvent == null || MyEvent.GetInvocationList().Length <= 0) return false;
            ieArgsOutput = MyEvent.Invoke();
            return true;
        }
